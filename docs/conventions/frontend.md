...
- 화면 문구, 버튼, 카드, 테이블 텍스트가 모바일/데스크톱에서 겹치지 않게 확인합니다.
...
- `components`: 여러 곳에서 재사용하는 UI입니다.
- `context`: 전역 상태 제공자입니다.
- `features`: 특정 기능에 묶인 코드입니다.
- `shared`: API 클라이언트, 공통 상수, 훅, 유틸입니다.
...
- API URL을 컴포넌트 내부에 여러 번 하드코딩하지 않습니다.
