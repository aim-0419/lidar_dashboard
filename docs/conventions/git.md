...
현재 프로젝트는 간소화 Git Flow를 사용합니다.

```text
작업 브랜치 -> dev -> main
```

- `dev`: 일반 기능 개발과 팀 통합 기준 브랜치
- `main`: 전체 검증을 통과한 현장 테스트·납품 가능 버전
- 작업 브랜치는 최신 `dev`에서 생성하고 PR 대상을 `dev`로 설정합니다.
- 현장 테스트나 납품 버전이 확정되면 `dev -> main` PR을 생성합니다.
- PR 병합 후 작업 브랜치는 로컬과 원격에서 삭제합니다.
